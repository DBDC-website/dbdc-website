-- Public registration forms run with the anon key. Inserts work, but
-- INSERT ... RETURNING requires SELECT privileges + a permissive SELECT policy,
-- which would expose all submissions. These SECURITY DEFINER functions perform
-- the parent + child inserts in one transaction and return only the new id.
