...
function articleMeta(article: ArticlePdf): string | null {
  ...
  return (
    <StaggerChildren as="ol" className="max-w-4xl space-y-6">
      {articles.map((article) => {
        ...
        const hasPdf = Boolean(article.href?.trim());
        const body = (
          <>
            <span
        ...
        return (
          <StaggerItem key={article.id} as="li">
            {hasPdf ? (
              ...
              <div className="flex gap-4 rounded-2xl border border-cream-200/90 bg-white/80 p-5 shadow-sm shadow-brand-900/[0.03] sm:gap-5 sm:p-6">
                {body}
              </div>
            )}
          </StaggerItem>
